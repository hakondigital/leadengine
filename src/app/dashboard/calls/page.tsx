...
export default function CallsPage() {
  ...
  const [searchResults, setSearchResults] = useState<TelnyxNumber[]>([]);
  ...
  if (planLoading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="w-6 h-6 animate-spin text-[var(--le-text-muted)]" /></div>;
  }
    ...
    <div className="min-h-screen">
      ...
      <div className="px-4 lg:px-6 py-6 space-y-6">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Hash className="w-4 h-4 text-[var(--le-accent)]" />
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {trackingNumbers.map((tn, i) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-3 rounded-[var(--le-radius-md)] border border-[var(--le-border-subtle)]"
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-[var(--le-bg-tertiary)]">
                        <Phone className="w-3.5 h-3.5 text-[var(--le-text-muted)]" />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[var(--le-text-primary)]">{tn.number}</p>
                        <p className="text-xs text-[var(--le-text-tertiary)]">{tn.label}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-medium text-[var(--le-text-secondary)]">{tn.totalCalls} calls</span>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <PhoneCall className="w-4 h-4 text-[var(--le-accent)]" />
              ...
              <div className="space-y-2">
                {calls.map((call, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex items-center justify-between py-3 ${hasTranscript ? 'cursor-pointer hover:bg-[var(--le-bg-tertiary)]/50 transition-colors rounded-md px-2 -mx-2' : ''}`}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-medium text-[var(--le-text-primary)]">{call.callerName}</p>
                              {call.linkedLead && (
                                <span className="flex items-center gap-0.5 text-[10px] text-[var(--le-accent)] cursor-pointer hover:underline">
                                  <ExternalLink className="w-2.5 h-2.5" />
                                  Lead
                                </span>
                              )}
                              {hasTranscript && (
                                <span className="flex items-center gap-0.5 text-[10px] text-[var(--le-accent)]">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-xs text-[var(--le-text-muted)]">{call.callerNumber}</span>
                              <span className="text-[10px] text-[var(--le-text-muted)]">via {call.trackingLabel}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 text-right">
                          <div>
                            ...
                            <p className="text-xs text-[var(--le-text-muted)] mt-0.5">{call.duration}</p>
                          </div>
                          <span className="text-[10px] text-[var(--le-text-muted)] w-16 text-right">{call.timestamp}</span>
                          {hasTranscript && (
                            isExpanded ? <ChevronUp className="w-4 h-4 text-[var(--le-text-muted)]" /> : <ChevronDown className="w-4 h-4 text-[var(--le-text-muted)]" />
                          )}
                        </div>
                      </div>
                      ...
                      {isExpanded && hasTranscript && (
                        ...
                        >
                          {call.aiSummary && (
                            <div className="p-3 rounded-[var(--le-radius-md)] bg-[rgba(91,141,239,0.06)] border border-[rgba(91,141,239,0.12)]">
                              <div className="flex items-center gap-1.5 mb-2">
                                <Sparkles className="w-3.5 h-3.5 text-[var(--le-accent)]" />
                                <p className="text-[10px] font-semibold text-[var(--le-accent)] uppercase tracking-wider">AI Summary</p>
                              </div>
                              <div className="text-xs text-[var(--le-text-secondary)] leading-relaxed whitespace-pre-wrap">
                                {call.aiSummary}
                              </div>
                            </div>
                          )}
                          {call.transcript && (
                            <div className="p-3 rounded-[var(--le-radius-md)] bg-[var(--le-bg-tertiary)] border border-[var(--le-border-subtle)]">
                              <div className="flex items-center gap-1.5 mb-2">
                                <FileText className="w-3.5 h-3.5 text-[var(--le-text-muted)]" />
                                <p className="text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Full Transcript</p>
                              </div>
                              <p className="text-xs text-[var(--le-text-tertiary)] leading-relaxed whitespace-pre-wrap">
                                {call.transcript}
                              </p>
                            </div>
                          )}
        ...
        {showProvisionModal && (
          ...
          >
            <motion.div
              ...
              className="bg-[var(--le-bg-secondary)] rounded-xl border border-[var(--le-border-subtle)] shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"
            >
              {/* Header */}
              <div className="flex items-center justify-between p-5 border-b border-[var(--le-border-subtle)]">
                <div>
                  <h2 className="text-lg font-bold text-[var(--le-text-primary)]">Add Tracking Number</h2>
                  <p className="text-xs text-[var(--le-text-muted)] mt-0.5">Search and provision a phone number for call tracking</p>
                </div>
                <button
                  onClick={() => setShowProvisionModal(false)}
                  className="p-1.5 rounded-md hover:bg-[var(--le-bg-tertiary)] text-[var(--le-text-muted)] transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {provisionSuccess ? (
                <div className="p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-[var(--le-text-primary)]">Number Provisioned!</h3>
                  <p className="text-sm text-[var(--le-text-secondary)] mt-1">
                ...
                <div className="p-5 space-y-5">
                  ...
                  <div className="space-y-3">
                    <p className="text-xs font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">
                      Step 1: Search for a number
                    </p>
                    <div className="flex gap-2">
                      <div className="flex-1">
                        <label className="text-xs text-[var(--le-text-secondary)] mb-1 block">Country</label>
                        <select
                          value={searchCountry}
                          onChange={(e) => setSearchCountry(e.target.value)}
                          className="w-full px-3 py-2 rounded-lg bg-[var(--le-bg-tertiary)] border border-[var(--le-border-subtle)] text-sm text-[var(--le-text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--le-accent)]"
                        >
                      ...
                      <div className="w-28">
                        <label className="text-xs text-[var(--le-text-secondary)] mb-1 block">Area Code</label>
                        <input
                          type="text"
                          value={searchAreaCode}
                          onChange={(e) => setSearchAreaCode(e.target.value)}
                          placeholder="e.g. 02"
                          className="w-full px-3 py-2 rounded-lg bg-[var(--le-bg-tertiary)] border border-[var(--le-border-subtle)] text-sm text-[var(--le-text-primary)] placeholder:text-[var(--le-text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--le-accent)]"
                        />
                      </div>
                  ...
                  {searchResults.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1.5 rounded-lg border border-[var(--le-border-subtle)] p-2">
                        {searchResults.map((num) => {
                          const isSelected = selectedNumber === num.phone_number;
                          const region = Array.isArray(num.region) && num.region[0]?.region_name;
                          const monthlyCost = num.cost && typeof num.cost === 'object' && 'monthly_cost' in num.cost
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-all ${
                                isSelected
                                  ? 'bg-[var(--le-accent-muted)] border border-[var(--le-accent)] ring-1 ring-[var(--le-accent)]'
                                  : 'hover:bg-[var(--le-bg-tertiary)] border border-transparent'
                              }`}
                            >
                              <div className="flex items-center gap-2.5">
                                <Phone className={`w-4 h-4 ${isSelected ? 'text-[var(--le-accent)]' : 'text-[var(--le-text-muted)]'}`} />
                                <div>
                                  <p className={`text-sm font-medium ${isSelected ? 'text-[var(--le-accent)]' : 'text-[var(--le-text-primary)]'}`}>
                                    {num.phone_number}
                                  </p>
                                  {region && (
                              ...
                              <div className="text-right">
                                {monthlyCost && (
                                  <span className="text-xs text-[var(--le-text-secondary)]">${monthlyCost}/mo</span>
                                )}
                                {isSelected && <CheckCircle className="w-4 h-4 text-[var(--le-accent)] mt-0.5" />}
                              </div>
                            </button>
                          );
                        })}
                  ...
                  {selectedNumber && (
                    ...
                    >
                      ...
                      <div className="p-3 rounded-lg bg-[var(--le-accent-muted)] border border-[var(--le-accent)]/20">
                        <p className="text-xs text-[var(--le-text-secondary)]">Selected number</p>
                        <p className="text-sm font-semibold text-[var(--le-accent)]">{selectedNumber}</p>
                      </div>
                      <div>
                        <label className="text-xs text-[var(--le-text-secondary)] mb-1 flex items-center gap-1">
                  ...
                  {/* Action */}
                  {selectedNumber && (
                    <div className="flex justify-end gap-2 pt-2 border-t border-[var(--le-border-subtle)]">
