    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-20 bg-[var(--le-bg-primary)]/80 backdrop-blur-xl border-b border-[var(--le-border-subtle)]">
        <div className="px-4 lg:px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-[var(--le-text-primary)] tracking-tight">ROI Dashboard</h1>
            <p className="text-sm text-[var(--le-text-tertiary)] mt-0.5">Track return on investment across all channels</p>
          </div>
          <div className="flex items-center gap-1 bg-[var(--le-bg-tertiary)] rounded-lg p-0.5">
            {periods.map((p) => (
              <button
                key={p.value}
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                  period === p.value
                    ? 'bg-[var(--le-bg-elevated)] text-[var(--le-text-primary)]'
      ...
      <div className="px-4 lg:px-6 py-6 space-y-6">
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">{stat.label}</p>
                      <p className="text-2xl font-bold text-[var(--le-text-primary)] mt-1">{stat.value}</p>
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-[10px] text-[var(--le-text-muted)]">vs prev</span>
                      </div>
                    </div>
                    <div
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-48 flex items-center justify-center text-[var(--le-text-muted)] text-sm">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <PieChart className="w-4 h-4 text-[var(--le-accent)]" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--le-border-subtle)]">
                    <th className="text-left py-2 pr-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Channel</th>
                    <th className="text-right py-2 px-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Spend</th>
                    <th className="text-right py-2 px-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Leads</th>
                    <th className="text-right py-2 px-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Won</th>
                    <th className="text-right py-2 px-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">Revenue</th>
                    <th className="text-right py-2 px-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">CPL</th>
                    <th className="text-right py-2 pl-4 text-[10px] font-semibold text-[var(--le-text-muted)] uppercase tracking-wider">ROI</th>
                  </tr>
                </thead>
                <tbody>
                  {channels.map((ch, i) => (
                    ...
                    >
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-[var(--le-text-primary)]">{ch.channel}</span>
                          {ch.trend === 'up' && <ArrowUpRight className="w-3 h-3 text-[#34C77B]" />}
                          {ch.trend === 'down' && <ArrowDownRight className="w-3 h-3 text-[#E8636C]" />}
                        </div>
                      </td>
                      <td className="py-3 px-4 text-right text-sm text-[var(--le-text-secondary)]">
                        {ch.spend > 0 ? formatCurrency(ch.spend) : '—'}
                      </td>
                      <td className="py-3 px-4 text-right text-sm font-medium text-[var(--le-text-primary)]">{ch.leads}</td>
                      <td className="py-3 px-4 text-right text-sm text-[#34C77B] font-medium">{ch.conversions}</td>
                      <td className="py-3 px-4 text-right text-sm font-semibold text-[var(--le-text-primary)]">{formatCurrency(ch.revenue)}</td>
                      <td className="py-3 px-4 text-right text-sm text-[var(--le-text-secondary)]">
                      ...
                      <td className="py-3 pl-4 text-right">
                        {ch.roi > 0 ? (
                          ...
                          <span className="text-xs text-[var(--le-text-muted)]">Organic</span>
                        )}
                      </td>
                    </motion.tr>
                  ))}
                ...
                <tfoot>
                  <tr className="border-t-2 border-[var(--le-border-subtle)]">
                    <td className="py-3 pr-4 text-sm font-bold text-[var(--le-text-primary)]">Total</td>
                    <td className="py-3 px-4 text-right text-sm font-bold text-[var(--le-text-primary)]">{formatCurrency(totalSpend)}</td>
                    <td className="py-3 px-4 text-right text-sm font-bold text-[var(--le-text-primary)]">{totalLeads}</td>
                    <td className="py-3 px-4 text-right text-sm font-bold text-[#34C77B]">{totalConversions}</td>
                    <td className="py-3 px-4 text-right text-sm font-bold text-[var(--le-text-primary)]">{formatCurrency(totalRevenue)}</td>
                    <td className="py-3 px-4 text-right text-sm font-bold text-[var(--le-text-primary)]">
